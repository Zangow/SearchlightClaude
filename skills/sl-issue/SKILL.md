...
2. **It's still accurate — checked over the anchors the plan's steps depend on.** Check only the `file:line` anchors in the plan's **`### Steps`** section; anchors cited anywhere else (approach, tests, risks, out of scope) are context, not dependencies — don't loop over them. Skip a path a step says it will **create** (a new migration, class, test) — its absence is expected. The anchor's prefix names the repo (`IntegrationService/…`, or `.claude/…` for a skills card); strip it, and the `:<line>` suffix, for `<path>`. An anchor with no repo prefix, or whose path does not exist as written (e.g. `domain/Foo.java`), resolves via `git -C "$SL_BASE_PATH/<repo>" ls-files "*<path>"`: exactly one match is its path; zero, or several you cannot tell apart, make it **UNRESOLVABLE** — list it separately and judge it by opening the candidates, never counting it stale or fresh on that alone. Then one call per repo lists every step path touched since the plan was posted:
   ```bash
   git -C "$SL_BASE_PATH/<repo>" fetch -q origin main
   git -C "$SL_BASE_PATH/<repo>" log --first-parent -m --since="<plan comment createdAt>" --name-only --format= origin/main -- <step paths…> | sort -u
   ```
   `--first-parent -m` is load-bearing: PRs land as merge commits, so a PR whose commits predate the plan but merged after it shows up only as the merge commit's diff — without these flags `git log` skips the old-dated branch commits and prints no files for the merge, and the stale anchor looks untouched.
   Every path printed is a **stale** anchor, and so is a resolved step path that no longer exists — open those and judge whether the step still applies. Also re-read the issue body/comments for scope changes **after** the plan's date, and re-check the plan's `**Change type:**` and `**Deploy needed:**` header claims — a capability the plan assumed was missing may have shipped in the meantime (or vice versa). **Adopt as-written only with no stale step anchor left unexplained and no scope change after the plan**; otherwise it is stale — name the anchors that moved.
3. **It's complete.** It must carry a requirements checklist and steps with `file:line` anchors. A plan with unresolved open questions in `### Risks & unknowns` that change *what* to build is not signed off — resolve those per step 2's ambiguity rule before implementing.
...
Only ambiguities that change *what* gets built warrant this, and only here, before implementation — after it, run straight through to the PR. Resolve pure implementation choices yourself. (If the ambiguities are the *point* of the exercise, that's `/sl-plan`.)
...
Otherwise, plan the change against the checklist **inline, in the main thread** — you have to load the relevant files to implement anyway, so a separate planning agent just pays to read the same context twice. Planning and authoring are **core roles — keep them on Opus** (this thread, or an `sl-core-worker` for any core work you hand off); reserve cheaper models for the review/verify panels downstream.

**Plan-review gate (contract-level changes only; skipped when a reviewed `sl-plan` plan was adopted).** Run it before writing code **only when** the plan changes a published contract/schema, an external integration contract, or persistence/migrations — the class of change where a wrong plan is expensive to unwind. Run it exactly as `_shared/review-gate.md` states (roster → dispatch → adjudicate → one delta round → ask); each reviewer's lens is: will this plan meet every checklist row? what will break? what's simpler? Hand them the issue, the requirements checklist and your written plan, and record the verdict + CONCERN ledger in your final report. If its step 5 fires while you cannot reach the user (a `sl-issues` child), return `BLOCKED:` with the surviving BLOCKERs and that step's options instead. For everything else, skip the gate — plans are cheap to revise mid-implementation, and the verification loop is the real safety net.
...
Apply it while planning: one named test per testable checklist row, with its level in the row's Evidence column. `./gradlew check` does not run a new AT, so run it yourself against a local stack (`scripts/run-acceptance.sh local` after `sl-start-env`) before handing off.
...
**Authoring ends here.** Run inline, the ship pipeline re-reads this whole authoring context on every tool call — measured runs spent ~90% of their tokens that way.
...
sl-ship then runs its normal pipeline — one `code-review --fix` pass (effort per `sl-ship` step 1) → `sl-verify` (one repair round, including the requirements-traceability pass) → PR. There is **no routine post-PR review step**: once the PR is open the pipeline stops editing code. And:
