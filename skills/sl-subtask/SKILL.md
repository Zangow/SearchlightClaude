...
- **Main stays green after every merge.** Merging cards 1..k in order must leave `./gradlew check` passing and the service working for every k. There is no CI (no `.github/workflows`): "green" means `./gradlew check` run locally and recorded on the PR — never plan a CI job into a child. Dark-ship or config-gate a half-built surface rather than landing a broken one.
