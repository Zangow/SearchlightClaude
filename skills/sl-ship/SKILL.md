...
> ⚠️ **In worktree mode, name the repo path in the FIRST invocation.** `code-review` forks a fresh agent that inherits the **session's** cwd, not yours — under `sl-issue`'s default worktree mode that is the original checkout at `$SL_REAL_BASE/IntegrationService`, not your worktree. Exporting `SL_BASE_PATH` does **not** fix this: `code-review` is a built-in, it never reads that variable, and an env var cannot change another process's working directory. So always pass the **absolute** worktree path — `$SL_BASE_PATH/IntegrationService`, i.e. `$SL_REAL_BASE/.sl-worktrees/issue-<n>-<slug>/IntegrationService` — in the args of the *initial* call. Correcting it afterwards via `SendMessage` works but costs a full re-run.
>
> **Sanity-check the result before accepting it.** Two different wrong-tree outcomes, both of which look like a normal review:
> - the main checkout is on a clean `main` → it reviews an **empty diff** and reports nothing to fix, which is indistinguishable from a clean review. Zero findings on a diff you know is large means it read the wrong tree — re-dispatch with the path, don't bank it as clean.
> - the main checkout is **dirty** → it adopts those uncommitted files as its review scope and `--fix` rewrites them. On 2026-08-19 that was 75 dirty files of the user's own work, caught seconds before the write phase (`Zangow/SearchlightClaude#1`). If the reported scope names files your diff doesn't touch, stop it (`TaskStop`) before it writes.
...
2. **Fall back to the built-in in its PR form.** ⚠️ The `code-review:code-review` **plugin is gone** — `~/.claude/plugins/installed_plugins.json` has read `{"plugins": {}}` since 2026-08-22, and the leftover `cache/` and `marketplaces/` directories under `~/.claude/plugins/` are marketplace clones, **not** an install. Do not invoke it; it is uninvokable and reaching for it just burns the rung. Instead re-invoke the **built-in** in its other, GitHub-anchored form: finish step 2, then at step 3 open the PR (`gh pr create`), run `gh pr ready <PR#>` (the reviewer refuses drafts), and call `Skill(skill: "code-review", args: "<effort> <full PR URL>")`. **Pass the full URL, not a bare number** — a bare number is resolved against the cwd repo. This form reads the PR from GitHub instead of a local tree, so it cannot land in the wrong checkout; it also does **not** write, so apply any **Critical/High** yourself in the worktree, re-run `./gradlew check` plus the targeted tests, and push. Say in the ship report that the review ran in PR form and what triggered it. This is a *substitute* for step 1, not an addition — it does not consume `sl-verify`'s repair round.
